...
export const getDirettoriTurnCountsWithTrend = () => {
  // Ottieni i dati dell'ultimo mese completo
  const lastMonthData = getDirettoriTurnCountsForPeriod('mese');
  ...
  const prevMonthData = direttoriData.filter(({ mese }) => {
    const dataMese = new Date(mese + '-01');
